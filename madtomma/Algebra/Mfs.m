...
BeginPackage["Madtomma`Mfs`Mfs`","Statistics`DataManipulation`","JMJUtils`BumpEtc`"]
...
mfsTypes::usage="mfsTypes is a list of the types of data related to the basic \
mfs type.  Various mfs data functions will apply to the elements in the \
class."



tfsRead::usage="tfsRead[file] returns an mfs data object containing all the \
...
removeQuotes::usage:=
    "removeQuotes[x] will remove any double quotes \" explicitly included in \
a string x.  If x is not a string then it is returned unchanged.";

stringRemoveLeadingTrailingBlanks::usage="stringRemoveLeadingTrailingBlanks[\
string] removes leading and trailing blanks from a string."



tfsParseDescriptorLine::usage=
    "tfsParseDescriptorLine[string] takes a TFS descriptor line as a string \
and returns a list consisting of the TFS key and its value.";

interpretTagValue::usage=
    "interpretTagValue[{\"tag\",val}] creates a variable tag and assigms it \
the value val.";



tfsFormatRules::usage=
...
tfsParseHeaderBlock::usage=
    "tfsParseHeaderBlock[file] returns the information in the header block of \
a TFS file as a structured list.  It is normally used inside readTfsTable.";

mfsInterpretKeys::usage=
...
mfsInterpretColumns::usage=
    "mfsInterpretColumns[mfsdata] creates a set of variables with values from \
the columns in an mfs (or related) data object. Any existing values are \
over-written.";
...
over-written.\nmfsInterpret[\"filename\"] creates such variables using data \
from a TFS file.";

interpretDescriptors::usage=
    "Obsolete function name.  Please use mfsInterpretKeys.";

mfsKeyValue::usage= 
    "mfsKeyValue[mfsdata,key] returns the value corresponding to a descriptor \
key (a string) in an mfs (or related) data object.";

keyValue::usage="Obsolete function name.  Please use mfsKeyValue.";

mfsColumnNames::usage =
    "mfsColumnNames[mfsdata] returns the list of column names in an mfs data \
(or related) object.";

columnNames::usage="Obsolete function name.  Please use mfsColumnNames.";

mfsKeyNames::usage=
    "mfsKeyNames[mfsdata] returns the list of key names in the header block \
of an mfs (or related) data object.";
...
mfsColumnValue::usage=
    "mfsColumnValue[mfsdata,row,key] extracts a value labelled by key in row \
...
mfsMember::usage=
    "mfsMember[mfsdata,key,targetset] extracts rows of an mfsdata object for \
which values labelled by key belong to targetset.";

mfsNotMember::usage=
    "mfsNotMember[mfsdata,key,targetset] extracts rows of an mfsdata object \
for which values labelled by key DO NOT belong to targetset.";

mfsRange::usage=
    "mfsRange[mfsdata,key,{min,max}] extracts rows of an mfsdata object for \
which values labelled by key lie within min and max. NOTE : min must be \
smaller than max !!!";

mfsDrop::usage = 
    "mfsDrop is an option for mfsInterpret that specifieds whether the first \
and last line should be dropped.";
...
keepTemporaryFile::usage="keepTemporaryFile is an option for tfsRead that \
causes a temporary file not to be deleted."



mfsAddKey::usage=
    "mfsAddKey[mfsdata,{\"KEY\",value}] returns the mfs (or related) data \
object mfsdata with an additional key and value.";

addDescriptor::usage="Obsolete function name.  Please use mfsAddKey.";

mfsEditKey::usage = "mfsEditKey[mfsdata,{key,newValue}] returns the mfs (or \
related) data object mfsdata with the value corresponding to the descriptor \
key replaced by newValue." 
...
mfsDeleteColumn::usage=
    "mfsDeleteColumn[qp,\"colname\"] returns an mfs object equal to qp but \
with the column labelled colname removed. A list of column names can also be \
given.";

mfsAddColumn::usage=
    "mfsAddColumn[qp,\"colname\",coldata] returns an mfs object equal to qp \
but with a new column labelled colname and containing data coldata added. A \
list of column names and columns of data can also be given.";

mfsReverse::usage=
    "mfsReverse[mfsdata] returns the mfs object with the rows of the main \
block of columns in reverse order.";

mfsColumnMatch::usage=
    "mfsColumnMatch[{qp1,qp2,...},{\"col1\",\"col2\",...}] tests whether a \
list of column namesmatch in all the mfs objects in which they appear.";

mfsMerge::usage="mfsMerge[{qp1,qp2,...}] nerges a list of mfs objects into a \
single one containing all the header and column information in each of them.  \
The column lengths must match."



matchColumns::usage=
    "matchColumns is an option for mfsMerge that gives a list of column names \
that must match in all the mfs objects in which they appear.  The value \
Automatic insists that all possible matches hold.";

mfsFixMAD8inconsistencies::usage="mfsFixMAD8inconsistencies[qp] returns a new \
mfs object with corrections for inconsistencies that may arise from it's \
having been generated by MAD Version 8."



mfsVersion2Update::usage="mfsVersion2Update[oldfile,newfile] will create a \
new version of a file in which names of functions in Version 1.x of the \
Madtomma`Mfs`Mfs` package are changed to their equivalents in Version 2.x."



mfsVersion2UpdateRules::usage=
    "mfsVersion2UpdateRules is a list of string replacement rules that will \
change names of functions in Version 1.x of the Madtomma`Mfs`Mfs` package to \
their equivalents in Version 2.x.";
...
mfsRange::badarg=
    "The first element of a range specification (`1`) should be smaller than \
the second one (`2`).";

mfsAddColumn::collength=
...
mfsMerge::badmatch="Some columns specified by the matchColumns option do not \
match."



mfsMerge::collength="Not all columns have the same length. mfs objects cannot \
be merged."
...
removeUnwantedLines[rawFile_String,filteredFile_String,unwanted_String,
    opts___Rule]:=
  Module[{oo},oo=OpenWrite[filteredFile];
    removeUnwantedLines[rawFile,oo,unwanted,opts];Close[oo]]

removeUnwantedLines[rawFile_String,oo_OutputStream,unwanted_String,
    opts___Rule]:=Module[{tt,ll},
		tt=OpenRead[rawFile];
		...
		While[Not[ll===EndOfFile],
			If[StringMatchQ[ToString[ll],"*"<>unwanted<>"*"],Null,
        WriteString[oo,ll<>"\n"]];
			ll=Read[tt,String]];
		Close[tt];
	]
...
tfsParseDescriptorLine[str_String]:=
  Module[{tp},
    tp=First[ReadList[StringToStream[str],{Word,Word,String},
          WordSeparators\[Rule]{" ","\"","\t","@"}]];
    Flatten[{First[tp],
        If[StringTake[
              tp\[LeftDoubleBracket]2\[RightDoubleBracket],{-1}]\[Equal]"s",
          StringReplace[
            stringRemoveLeadingTrailingBlanks[Last[tp]],{"\" "\[Rule]"",
              " \""\[Rule]"","\""\[Rule]""}],
          ReadList[StringToStream[Last[tp]],Number]]}]]
...
Options[tfsParseHeaderBlock]={mfsVerbose\[Rule]False,
      tfsFormatRules->{"%e"->Number,"%hd"->Number,"%d"->Number,"%lf"->Number,
          "%f"->Number,"%le"\[Rule]Number,"%s"->Word,"%16s"->Word,
          "%32s"->Word,"~"->Word}};
...
tfsParseHeaderBlock[tfsFile_String,opts___Rule]:=
  Module[{tt,headers,keyvals,lastKeyPos,fmtrules},
		...
		headers = ReadList[StringToStream[ll],Word,WordSeparators->{" ","*",","}];
		Close[tt];
		tt=OpenRead[tfsFile]; (* get column format info *)
		ll=Find[tt,"$",AnchoredSearch->True];
		fmts = ReadList[StringToStream[ll],Word,
          WordSeparators->{" ","$",","}]/.Flatten[fmtrules];
...
tfsRead[tfsFile_String,opts___Rule]:=
  Block[{tempFile,tt,hh,cols,particles,turns},
		...
		If[verbose,
			Print["Header block:\n",First[hh] , 
				"\nColumns labels:  ",hh\[LeftDoubleBracket]-2\[RightDoubleBracket] ,
				"\nColumn formats:  ",
	  ...
	  SetStreamPosition[tt,Last[hh] ];
		cols =Map[removeQuotes,
        ReadList[tt,hh\[LeftDoubleBracket]-3\[RightDoubleBracket]],{-1}];
		Close[tt];
		If[keepTemp,
			Print["Temporary file not deleted: ",tempFile],
			DeleteFile[tempFile]
		]; 
		mfs[ First[hh],hh\[LeftDoubleBracket]-2\[RightDoubleBracket],cols]
		]
...
interpretTagValue[{tag_String,value_}]:=Block[{temp,newtag},
    temp=value;
    newtag=StringReplace[tag,{"_"\[Rule]"$$"}];
    If[newtag\[NotEqual]tag,
      Message[interpretTagValue::changename,tag,newtag]];
...
mfsInterpretKeys[qp_mfs,
    opts___]:=(verbose=mfsVerbose/.{opts}/.Options[mfsInterpret];
    interpretTagValue /@ First[qp];
    If[verbose,Print["Variables (re-)assigned: ",mfsKeyNames[qp]]])



mfsInterpretColumns[qp_mfs,opts___]:=Block[{verbose,drop},
    verbose=mfsVerbose/.{opts}/.Options[mfsInterpret];
    drop=mfsDrop/.{opts}/.Options[mfsInterpret];
    If[drop,
        interpretTagValue/@
            Transpose[{qp\[LeftDoubleBracket]-2\[RightDoubleBracket],
                Transpose[Rest[Drop[Last[qp],-1]]]}];,
        interpretTagValue/@
            Transpose[{qp\[LeftDoubleBracket]-2\[RightDoubleBracket],
                Transpose[qp]}];]
      
      If[verbose,Print["Variables (re-)assigned: ",mfsColumnNames[qp]]]]



Options[mfsInterpret]={mfsVerbose\[Rule]False,mfsDrop\[Rule]True};

mfsInterpret[qp_mfs,opts___]:=(mfsInterpretKeys[qp,opts];
    mfsInterpretColumns[qp,opts])

mfsInterpret[fn_String,opts___]:=mfsInterpret[tfsRead[fn],opts]
...
mfsKeyValue[qp_mfs,key_String]:=Module[{qp1,pp},
      ...
      Extract[qp1,MapAt[(#+1)&,First[pp],-1] ]]
	]
...
mfsColumn[qp_mfs,key_String]:=Module[{},
		...
		If[pos=={},(Message[mfsColumn::notfound,key];Null),
...
mfsMember[qp_mfs,key_String,targetset_List]:=
  mfsSelect[qp,(MemberQ[targetset,mfsColumnValue[qp,#,key]])&]

mfsNotMember[qp_mfs,key_String,targetset_List]:=
  mfsSelect[
    qp,\[InvisibleSpace]!(MemberQ[targetset,mfsColumnValue[qp,#1,key]])&]
...
mfsRange[qp_mfs,key_String,{min_,max_}]:=
    ...
    mfsSelect[
      qp,( (mfsColumnValue[qp,#,key]\[GreaterEqual]
                min) && (mfsColumnValue[qp,#,key]\[LessEqual]max))&],
    Message[mfsRange::badarg,min,max]]
...
mfsDeleteKey[qp_mfs,key_String]:=
	Module[{qp1,pp},
		...
		If[pp=={},
			(Message[mfsKeyValue::notfound,key];Null),
        ReplacePart[qp,Select[qp1,(Not[First[#]==key])&],1]
	]];
...
mfsEditKey[qp_mfs,{key_String,newValue_}]:=Module[{qp1,pp},
		...
		If[pp=={},
			(Message[mfsKeyValue::notfound,key];Null),
        ReplacePart[qp,ReplacePart[qp1,newValue,MapAt[(#+1)&,First[pp],-1]],
          1]
	]];
...
mfsDeleteColumn[qp_mfs,colname_String]:=
	Module[{colpos},
		...
		mfs[
			First[qp],
			Drop[qp\[LeftDoubleBracket]-2\[RightDoubleBracket],First[colpos]],
...
mfsColumnMatch[qp:{__mfs},mcol_String]:=
	Module[{},
		Equal@@(mfsColumn[#,mcol]& /@ (Select[qp,
              MemberQ[mfsColumnNames[#],mcol]&]))
			]
	...
	Module[{matchCols,colnameswanted,allcolnames},
		...
		mfs[
            ...
            Extract[Transpose[MapThread[Join,Last/@qp]],
              First/@(Position[allcolnames,#]&/@colnameswanted)]]
				],
...
mfsFixMAD8inconsistencies[qp_mfs]:=Module[{kv,ch},
    kv=First[qp]/.{
          ...
          {"PY",py_}\[Rule]{"PYC",py},
          {"T",t_}\[Rule]{"TC",x},
          {"DELTAP",pt_}\[Rule]{"PTC",pt}
          };
    ch=qp\[LeftDoubleBracket]-2\[RightDoubleBracket]/.{"DELTAP"\[Rule]"PT"};
    ReplacePart[
      ReplacePart[qp,kv,{1}],
      ch,{-2}
      ]
...
mfsVersion2UpdateRules={"interpretDescriptors"\[Rule]"mfsInterpretKeys",
    "keyValue"\[Rule]"mfsKeyValue","columnNames"\[Rule]"mfsColumnNames",
    "descriptorNames"\[Rule]"mfsKeyNames","colValue"\[Rule]"mfsColumnValue",
    "addDescriptor"\[Rule]"mfsAddKey","editDescriptor"\[Rule]"mfsEditKey",
    "deleteDescriptor"\[Rule]"mfsDeleteKey","symplecticJ"\[Rule]"SymplecticJ"}



mfsVersion2Update[oldFile_String,newFile_String]:=
  stringReplaceInFile[oldFile,newFile,mfsVersion2UpdateRules]
...
Protect[mfsTypes,removeUnwantedLines,tfsParseDescriptorLine,interpretTagValue,\
(*tfsInterpretDescriptorLine,*)tfsParseHeaderBlock,mfsColumn,mfsSelect, \
mfsMember, \
mfsRange,mfsReverse,mfsAddKey,mfsKeyNames,mfsColumnNames,mfsInterpretKeys,\
mfsKeyValue,mfsColumnValue,mfsEditKey,mfsDeleteColumn,mfsAddColumn,mfsMerge,\
mfsDeleteKey,
	addDescriptor,editDescriptor,interpretDescriptors,deleteDescriptor, \
colValue, keyValue,columnNames,descriptorNames
]
...
If[$Remote,Print["Remote kernel: MfsPalette.nb will not be opened"],
  NotebookOpen[ToFileName[packageSourceDirectory["Mfs"],"MfsPalette.nb"]]]

Print["Version 2.21 of Madtomma`Mfs`Mfs` loaded.  Note function name changes \
