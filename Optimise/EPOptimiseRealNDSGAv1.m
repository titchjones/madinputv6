...
NDSGA::usage="EPOptimise[Objective Function, No Of Chromosomes, Iterations, Prob. of Crossover, Prob. of Mutation, Genes per Chrom., Start of range, End of range, Options (see EPOptimise[Options])]\nEPOptimise is a MAXIMISING Function!";
...
FitnessAll[list_,m_]:=(SaveFit[m,#]&/@list)

\!\(CumulativeProbabilities[list_List, m_List] := Block[{a}, \[IndentingNewLine]a = DominationRank[list, m]\^2; \[IndentingNewLine]Rest[FoldList[Plus, 0, a/Total[a]]]\[IndentingNewLine]]\)
...
Dominates[p_,q_,m_]:=Block[{},
    pans=SaveFit[#,p]&/@m;
    qans=SaveFit[#,q]&/@m;
    And@@{And@@(#\[LeftDoubleBracket]1\[RightDoubleBracket]>#\
\[LeftDoubleBracket]2\[RightDoubleBracket]&/@Transpose[{pans,qans}]),
        And@@(#\[LeftDoubleBracket]1\[RightDoubleBracket]\[GreaterEqual]#\
\[LeftDoubleBracket]2\[RightDoubleBracket]&/@Transpose[{pans,qans}])}
    ]
...
ObjectiveSort[I_,mfunc_]:=Module[{ans},
    ans=SaveFit[mfunc,#]&/@I;
    I\[LeftDoubleBracket]Ordering[ans]\[RightDoubleBracket]
    ]



CrowdingDistanceAssignment[P_List,m_List]:=
  Module[{l=Length[P],I=P,Idistance=Table[0,{Length[P]}]},
    Block[{mfunc=#},
          I=ObjectiveSort[I,mfunc];
                ...
                ReplacePart[Idistance,
                  Idistance\[LeftDoubleBracket]
                      i\[RightDoubleBracket]+(SaveFit[mfunc,
                          I\[LeftDoubleBracket]i+1\[RightDoubleBracket]]-
                        SaveFit[mfunc,
                          I\[LeftDoubleBracket]i-1\[RightDoubleBracket]]),i];
            ,{i,2,l-1}]]&/@m;
    Idistance
    ]
...
DominationRank[P_List,m_List]:=Module[{P2},
...
DominationSorting[P_,distances_,m_]:=Module[{ranks},
    ...
    P\[LeftDoubleBracket]Ordering[Transpose[{distances,ranks}],All,
        PartialOrder[#1,#2]&]\[RightDoubleBracket]
    ]
...
selectionmethods={Global`Roulette,Global`Tournament}
...
\!\(newvalue[input_List] := \(Min[#] + LimitedGaussianSample[\(Max[#] - Min[#]\)\/2, \(Max[#] - Min[#]\)\/6, {\(-10\), 10}] &\) /@ Transpose[input]\)
...
Crossover[datalist_,crosslist_]:=
  Block[{a={},b,c,pos,c1last,c2last,c1new,c2new,lengths},
    If[Length[crosslist]>0,
      ...
      c=datalist\[LeftDoubleBracket]#\[RightDoubleBracket]&/@a;
      newvalue[#]&/@
        Partition[Join[c,c\[LeftDoubleBracket]{1}\[RightDoubleBracket]],2,
          1],{}]]
...
ReinsertElitist[olddata_,newdata_,Null___]:=
  Block[{a,random=RandomUnion[Length[newdata]],positions},
    a=olddata;
    positions=
      Ordering[SaveFit[#]&/@olddata]\[LeftDoubleBracket]
          Range[Length[newdata]]\[RightDoubleBracket]\[LeftDoubleBracket]
...
Dominates[p_,q_,m_]:=Dominates[p,q,m]=Block[{},
      pans=SaveFit[#,p]&/@m;
      qans=SaveFit[#,q]&/@m;
      Or@@{Or@@(#\[LeftDoubleBracket]1\[RightDoubleBracket]>#\
\[LeftDoubleBracket]2\[RightDoubleBracket]&/@Transpose[{pans,qans}]),
...
FindNonDominatedFront[P_,m_]:=Block[{Pdash={}},
    AppendTo[Pdash,P\[LeftDoubleBracket]1\[RightDoubleBracket]];
    Block[{a=#},
          
          If[And@@(If[
                      Dominates[a,
                        Pdash\[LeftDoubleBracket]#\[RightDoubleBracket],m],
                      Drop[Pdash,{#}];True,False]&/@Range[Length[Pdash]]),
            AppendTo[Pdash,a]]]&/@Rest[P];
    Pdash
    ]
...
FastNonDominatedSort[Pin_,m_]:=Block[{P=Pin,F={},i=0},
    While[Length[P]>0,
      ...
      P=Complement[P,F\[LeftDoubleBracket]-1\[RightDoubleBracket]];
      i++;
      ];
    F
    ]
...
\!\(CrowdingDistanceAssignment[P_List, m_List] := Module[{Idistance = Table[0, {Length[P]}]}, \[IndentingNewLine]\(Block[{mfunc = #}, \[IndentingNewLine]ans = \(SaveFit[mfunc, #] &\) /@ P; \[IndentingNewLine]Idistance += MapIndexed[Min[Drop[#1, #2]]\^2 &, Outer[Abs[Subtract[##]] &, ans, ans]]] &\) /@ m; \[IndentingNewLine]Idistance\[IndentingNewLine]]\)
...
DominationSorting[P_,distances_,m_]:=Module[{ranks},
    ranks=DominationRank[P,m];
    P\[LeftDoubleBracket]Ordering[Transpose[{ranks,distances}],All,
        PartialOrder[#1,#2]&]\[RightDoubleBracket]
    ]

SelectionTournament[listin_,funcs_,number_,opts___]:=
  Block[{participants,list=listin},
    tour=Global`TournamentNumber/.{opts}/.{Global`TournamentNumber\[Rule]
            Ceiling[number/5]};
    If[printedsetting=!=True,
      Print["Using TournamentNumber \[Rule] "<>ToString[tour]];
      printedsetting=True];
    rutters=Block[{ran=Table[Random[Integer,{1,Length[list]}],{tour}]},
            participants=list\[LeftDoubleBracket]ran\[RightDoubleBracket];
            
            a=DominationSortingList[participants,
                CrowdingDistanceAssignment[participants,funcs],funcs];
            (*list=
                  Drop[list,{ran\[LeftDoubleBracket]
                        a\[LeftDoubleBracket]1\[RightDoubleBracket]\
\[RightDoubleBracket]}];*)
            
            breedinglist={Sequence@@breedinglist,
                ran\[LeftDoubleBracket]
                  a\[LeftDoubleBracket]1\[RightDoubleBracket]\
\[RightDoubleBracket]};
            
            participants\[LeftDoubleBracket]
              a\[LeftDoubleBracket]1\[RightDoubleBracket]\[RightDoubleBracket]\
]&/@Range[number];
    (*breedinglist=
  ...
  Block[{participants,list=listin},
    breedinglist=
      DominationSortingList[list,CrowdingDistanceAssignment[list,funcs],
          funcs]\[LeftDoubleBracket]Range[number]\[RightDoubleBracket];
    list\[LeftDoubleBracket]breedinglist\[RightDoubleBracket]
    ]
...
NDSGA[funcs_List,nochroms_,iters_,pc_:0.25,mr_:0.01,nogenes_,start_,end_,
    opts___]:=
  Block[{a,probs,b,c,alldata,current,best={-\[Infinity],0},R,Q,N=10,t=0},
...
;
              ...
              Global`SelectionMethod/.{opts}/.{Global`SelectionMethod\[Rule]
                    Tournament}]]];printedsetting=False;
...
\[Rule]Elitist}]]],
      If[ruttingpopulationsize>nochroms,
        Print[
          "Using Reinsertion Method \[Rule] Fitness (BreedingPool > Population)"]\
;
        reinsertmethod=EPOptimise`Private`ReinsertFitness,
        Print[
          "Using Reinsertion Method \[Rule] Pure (BreedingPool == Population)"]\
;
    ...
    If[startgene===Null,P=CreateChrom[nochroms,nogenes,start,end],
      P=Join[{startgene},CreateChrom[nochroms-1,nogenes,start,end]]
      ];
    AppendTo[rawdata,P];
    Print["Starting population = ",P];
    Q=P;
    Do[
      If[t>0,
        R=Join[P,Q];
        ...
        i=1;P={};
        While[
          Length[P]+Length[F\[LeftDoubleBracket]i\[RightDoubleBracket]]<
            nochroms,
          P=Join[P,F\[LeftDoubleBracket]i\[RightDoubleBracket]];
          i++;
          ];
        P=
          Join[P,DominationSorting[
                ...
                CrowdingDistanceAssignment[
                  F\[LeftDoubleBracket]i\[RightDoubleBracket],funcs],
                funcs]\[LeftDoubleBracket]Range[
                nochroms-Length[P]]\[RightDoubleBracket]];
        (*Print["P = ",P];*)
        ];
      (*P=Q;*)
      SeedRandom[];
      breedinglist={};
      ruttingpopulation=
        SelectionTournament[P,funcs,ruttingpopulationsize,opts];
      (*Print["breedinglist = ",breedinglist];
      ...
      If[Length[breedinglist]>1,
        ...
        newpopulation=Crossover[breedingpopulation,crossoverlist];,
        crossoverlist={1};
      ...
      Q=ReinsertElitist[P,newpopulation,funcs,start,end];
      
      (*Print["New population = ",Q];*)
      
      Q=Mutate[Q,start,end,mr];
