...
EPOptimiseReal::usage="EPOptimiseReal[Objective Function, No Of Chromosomes, Iterations, Prob. of Crossover, Prob. of Mutation, Start figure (Default: 2), No. of Genes, Start of range, End of range, Options (see EPOptimiseReal[Options])]\nEPOptimiseReal is a MAXIMISING Function!";
...
randomnumbers[number_]:=(RandomReal[{0,1},number])
...
FitnessAll[list_,rawdata_,fitdata_]:=Map[If[Position[Most[rawdata],#]=!={},fitdata[[Sequence@@Position[Most[rawdata],#][[1]]]],FitnessValue[#]]&,list]


CumulativeProbabilities[list_,rawdata_,fitdata2_]:=Block[{a},
a=FitnessAll[list,rawdata,fitdata2];
...
EPOptimiseReal[function_,nochroms_,iters_,pcstart_:0.25,mrstart_:0.01,msstart_:2,nogenes_,start_,end_,opts___]:=
Block[{a,probs,b,c,alldata,current,best={-\[Infinity],{}},dynamicgraphics,startgene,converge,selectmethod,ruttingpopulationsize,reinsertmethod,population,breedingrate,mutationrate,cp,ruttingpopulation,breedingpopulation,newpopulation,keepbest,bestpos,minrawdata,mutationsize,mutatemethod,crossovermethod,dynamicupdate},
...
Global`Gaussian]&[Global`MutationMethod/.{opts}/.{Global`MutationMethod->"Gaussian"}]]];
keepbest=Global`KeepBest/.{opts}/.{Global`KeepBest->False};
randomseed=Global`Seed/.{opts}/.{Global`Seed->False};
If[randomseed===False,SeedRandom[],Print["Using Random Seed: "<>ToString[randomseed]];SeedRandom[randomseed]];
...
cp=CumulativeProbabilities[population,rawdata,fitdata];
...
population=Mutate[population,mr,start,end,mutatemethod,ms];
...
If[i==1,
If[dynamicgraphics,
Print[Dynamic[Refresh[Madtomma`MADInput`MADInput`mfsPlot[{{Range[Length[fitdata]],fitmeasure/@fitdata},{Range[Length[fitdata]],meanmeasure/@fitdata}},PlotRange->All,ImageSize->400,Frame->True],TrackedSymbols->{i}]]];];
If[dynamicupdate,Print[Dynamic[i],". New Fit Value \[Rule] ",Dynamic[minvalue],"\tRaw Value \[Rule] ",Dynamic[mindata],"\tBreedingRate \[Rule] ",Dynamic[pc],"\tMutationRate \[Rule] ",Dynamic[mr],"\tMutationSize \[Rule] ",Dynamic[ms]]];
];
